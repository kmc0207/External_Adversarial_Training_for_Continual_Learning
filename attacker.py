...
class PGDAttacker():
    def __init__(self, num_iter, epsilon, step_size, kernel_size=15, prob_start_from_clean=0.0, translation=False, device='cuda:0',num_classes = 1000):
    ...
    def _create_random_target(self, label,high):
        label_offset = torch.randint_like(label, low=0, high=high)
        return (label + label_offset) % high

    def attack(self, image_clean, label, model, original=False):
        if original:
            target_label = label
        else:
            target_label = self._create_random_target(label,self.num_classes)
